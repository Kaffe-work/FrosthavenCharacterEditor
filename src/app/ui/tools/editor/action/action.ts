...
import {
  ...
  ActionType,
  ActionValueType
} from "src/app/game/model/data/Action";
...
export class EditorActionComponent implements OnInit {
  ...
  ActionTargets: ActionTarget[] = Object.values(ActionTarget);
  ...
  ngOnInit(): void {
    //action targeting
    if (!this.action.targeting) {
      this.action.targeting = {};
    }

    // Ensure that action.targeting.target and action.targeting.range are initialized
    if (this.action.targeting.target === undefined) {
      this.action.targeting.target = 0; // You can provide a default value if needed
    }

    if (this.action.targeting.range === undefined) {
      this.action.targeting.range = 0; // You can provide a default value if needed
    }
    if (this.action.targeting.condition === undefined) {
      this.action.targeting.condition = ''; // You can provide a default value if needed
    }
    ...
    console.log('Component initialized. Action:', this.action);
  }
  ...
  optionsArray(length: number): number[] {
  ...
  change() {
    ...
    console.log('Action Target Changed:', this.action.targeting);
  }
